...
export default function AIGrowingTipsScreen() {
  ...
  React.useEffect(() => {
    const initialPrompt = 'What are some best practices and tips for successful crop cultivation on a small farm?';
  ...
  const saveMessage = async (role: 'user' | 'assistant', content: string) => {
    try {
      ...
      const { error } = await supabase.from('ai_conversations').insert({
        ...
        role,
        content,
        conversation_type: 'growing_tips',
      });
  ...
  const handleBack = () => {
    console.log('Growing Tips: Back button pressed');
  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitle}>💡 Growing Tips</Text>
      ...
      <LinearGradient colors={['#2D5016', '#4A7C2C', '#6BA542']} style={styles.gradient}>
        ...
        <View style={styles.inputContainer}>
          <View style={styles.inputRow}>
            <TextInput
              style={styles.input}
              placeholder="Ask for specific growing tips..."
